...
Banach space admits a Karhunen-Lo\'{e}ve representation $$
...
The result is a shit-tonne easier to parse of we make $B$ a separable
...
it's prototypical, we typically do not work with the Karhunen-Lo\'{e}ve basis
...
Karhunen-L\'{o}eve basis or some other orthogonal basis expansion. 
...
are _Markovian_. The Markov property, in general, says that if you for every^[Not every. You do this for nice sets. See Rozanov's book on Markov random fields if you care.] set of 
...
sure the posteriors exist. The first one say "don't grow too fast" too the
...
sure the isn't [zero](https://www.youtube.com/watch?v=3wk7C64kaP4). The third
...
The argument that gets us here is really cute so I'll sketch it below.
...
pairings. This makes sense because^[I said before that $V_m$ wasn't smooth
enough. Now I'm saying it is. What gives? The answer will surprise
and alarm you! The problem is that the natural embedding degrades as $m$ 
grows. We use below that $\|v_m\|_{H^{1+\epsilon}} \leq C
h^{-1-\epsilon}\|v\|_{L^2}$, where $h$ is inversely related to $m$. This 
means that we can't get nice uniform-in-$m$ bounds 
on anything and life would be very hard. So we use $B = L^2$.] 
$$
...
Finally, to bound $\|u - z^{(m)}\|_{L^2}$ use one of my favourite arguments.
...
to the ones we've already covered. And then it would just 
