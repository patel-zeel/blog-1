...
matrix and then just absolutely _slam_ it into our filthy little Cholesky context.
...
class) is that it may end up being quite delicate to have this volume of auxillary  
information^[Permuations, cholesky, etc] in a pytree while making everything differentiate 
...
of differentiable operations on the elements of the Cholesky factor ($\log(|A|)$)
and triangular linear solves $L^{-1}b$ and $L^{-T}c$.
...
there's a lot of code to write and, potentially, a lot of technical debt to accrue).
...
We don't really need to make that choice yet. So we won't. But personally, I like
option 1. I expect everyone else on earth would prefer option 3. But for densities 
that see a lot of action, it would make quite a bit of sense to consider making
the density a primitive when it has a complex derivative (_à la_ option 2).
